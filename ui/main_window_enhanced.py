...
class PlotDialog(QDialog):
    ...
    def _plot_gray_histograms(self, original, encrypted, decrypted, image_name):
        ...
        ax1.set_title(f'原始图像直方图 - {image_name}' if image_name else '原始图像直方图')
        ax1.set_xlabel('像素值')
        ax1.set_ylabel('频数')
        ax1.set_xlim(0, 255)
    ...
    def _plot_color_histograms(self, original, encrypted, decrypted, image_name):
        ...
        for row, (img, title) in enumerate(zip(images, titles)):
            for col, (color, name) in enumerate(zip(colors, color_names)):
                ...
                if row == 0:
                    ax.set_title(f'{title} - {name}通道')
                else:
                    ax.set_title(f'{name}通道')
                ax.set_xlabel('像素值')
                ax.set_ylabel('频数')
                ax.set_xlim(0, 255)
    ...
    def plot_correlation_scatter(self, image: np.ndarray, image_name: str = "", 
        ...
        h, w = img.shape
        sample_size = min(5000, h * w // 2)  # 采样数量
        ...
        ax1.set_title(f'水平方向\n相关系数: {corr_h:.6f}')
        ax1.set_xlabel('像素值 (x)')
        ax1.set_ylabel('像素值 (x+1)')
        ax1.grid(True, alpha=0.3)
        ...
        ax2.set_title(f'垂直方向\n相关系数: {corr_v:.6f}')
        ax2.set_xlabel('像素值 (y)')
        ax2.set_ylabel('像素值 (y+1)')
        ax2.grid(True, alpha=0.3)
        ...
        ax3.set_title(f'对角方向\n相关系数: {corr_d:.6f}')
        ax3.set_xlabel('像素值 (x,y)')
        ax3.set_ylabel('像素值 (x+1,y+1)')
        ax3.grid(True, alpha=0.3)
        
        title_text = f'{image_name} - ' if image_name else ''
        title_text += '原始图像' if direction == 'original' else '加密图像'
        title_text += ' 相邻像素相关性分析'
...
class EnhancedMainWindow(QMainWindow):
    ...
    def create_top_panel(self) -> QWidget:
        ...
        # 显示默认密钥（只读）
        layout.addWidget(QLabel("默认密钥:"))
        self.key_display = QLabel("(未选择算法)")
    ...
    def on_algorithm_changed(self, algo_name: str):
        ...
        if algo_name in self.algorithms:
            ...
            if default_key is not None:
                self.key_display.setText(str(default_key))
            else:
                self.key_display.setText("(未设置)")
    ...
    def start_batch_evaluation(self):
        """开始批量评估"""
        # 使用算法的默认密钥
        key = self.controller.algorithm.get_default_key()
        
        if key is None:
            QMessageBox.warning(self, "警告", "当前算法未设置默认密钥！")
            return
    ...
    def show_histograms(self):
        ...
        # 创建对话框并显示直方图
        dialog = PlotDialog(self, title="图像直方图分析")
    ...
    def show_correlation(self):
        ...
        # 创建对话框并显示相关性散点图
        dialog = PlotDialog(self, title="相邻像素相关性分析")
